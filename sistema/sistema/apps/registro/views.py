...
def login_view_Estudiante(request):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			try:
				r=Estudiante.objects.get(email=email,password=password)
				elid=int(r.id)
				estu=request.session['estudiante']=r.id
...
def login_view_Docente(request):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			password = form.cleaned_data['password']
			try:
				r=Docente.objects.get(email=email,password=password)
				elid=int(r.id)
				estu=request.session['docente']=r.id
